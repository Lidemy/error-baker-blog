...
工作一陣子之後發現其實大部分的工作時間都不是從 0 開始寫程式，反而會是在維護或是整理之前的 code。這個時候遇到的困難常常不是這個 bug 該怎麼解，而會是花費很多的心力在看前人留下的 code。這時候就會發現其實每個人對 class 的命名都有些不一樣，常常有種雖然好像看的但卻沒有一個規則的感覺，所以今天就來探討一些 CSS 的命名慣例來讓我們的 class 看起來不會那麼奇怪吧！
...
首先呢！跟其他語言的變數一樣的是給自己的 class 一個有語意的名字，而且不要隨意進行縮寫，如果真的要縮寫的話要注意那個縮寫會是大家都看得懂的，如果不確定的話就不要用。例如他是一個按鈕那就叫他 `button` ，頂多縮寫成約定俗成的 `btn`，不要縮寫成沒人知道是什麼東西的 `b` 之類的。
...
Element 是 Block 的字元素，可以看作是整個 Block 的內容們，命名原則為 `＿＿`之接 Element 的名稱，他有著以下的特性：
