...
这两个关键是 **任何编辑器** 的第一课。让我们回想一下当初是怎么开始编辑文字文件的。我猜我们大多数人第一次都是用内建的文字编辑器编辑 `.txt` 檔。
...
与其死记，你应该尽可能多地使用它们，直到有一天你不需要再查 Vim 的文件或 Google。这就像你在学你的第一组快捷键 `ctrl + c` 和 `ctrl + v` 一样，只是 Vim 的世界里有更多快捷键（它们很酷、很神奇）。而这些技能会跟你一起成长。我真的很喜欢这种感觉，就像你在玩一个 RPG 游戏，一开始拿著一把不怎么樣（也许很爛）的武器。但最后，它变成了传奇武器，并找到了它的主人。
